...
public class Menu extends JMenuBar {
	...
	/*
	 * För att kunna använda tangentbord-kommandon för att nå meny-alternativ, så
	 * anropar man
	 ...
	 * Så även fast JMenuItem är något som ligger inuti en JMenu, såsom våra
	 * kodexempel
	...
	private JComponent getComponentByName(String name) {
		 ...
		 * getComponents() returnerar en array med samtliga barn-komponenter
		 * (Component).
		 * Använd en for-each loop för att loopa över våran Menu-klass samtliga
		 * barn-komponenter.
		 */
		...
		/*
		 * 2. Vi är egentligen säkra på att alla "närmaste" barn-komponenter till våran
		 * Menu-klass
		 * kommer vara JMenu-objekt, så vi kan tryggt "casta" varje objekt som loopas
		 * igenom
		 * till ett JMenu-objekt. (egentligen kan vi casta till JMenuItem också. Hur
		 * kommer det sig tror du?)
		 */

		// TODO

		/*
		 * 3. Varför vill vi casta till ett JMenu-objekt? Jo, för att kunna anropa
		 * metoden
		 * getText() och jämföra med parametern "name". Om så är fallet,
		 * då har vi hittat våran komponent, och kan returnera den. Kom ihåg att INTE
		 * använda
		...
		/*
		 * 4. Ok, med den kod som skrivits fram till nu så kan vi hitta alla
		 * JMenu-objekt som är
		 * addade direkt till våran JMenuBar. Men hur gör vi för att nå JMenu-objekt som
		 * är addade
		 ...
		 * så vi kan på precis samma sätt som innan köra en for-each loop. Denna loop
		 * ska
		 ...
		 * Men en till orsak är att JMenu är en subklass till JMenuItem, så att försöka
		 * casta
		 ...
		 * Eftersom JMenu ärver från JMenuItem, och eftersom getText(), vilket är den
		 * metod vi behöver,
		 * är en metod som tillhör en föräldraklassen till JMenuItem
		...
		/*
		 * 7. Om inga av dessa vilkor har kunna uppfyllas, då måste metoden returnera
		 * null.
		 */
